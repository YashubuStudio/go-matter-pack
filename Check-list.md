...
- コミッショニング対応としてオンボーディングペイロード/運用クレデンシャルのインポート状態を保持する `internal/commission/state.go` を追加し、計画リストの8を完了に更新。
- 計画リストの7について機能/全体/構成/デバッグ/進捗の各確認を行い、制作完了状態を維持。
- 計画リストの8について機能/全体/構成/デバッグ/進捗の各確認を行い、コミッショニングの実行/CLI導線が未整備のため作業中状態へ更新。
